...
- 失败重试最多 3 次`;
    ...
    const ghostDiagnostics = diagnostics.filter(
...
3. 导出订单数据 (Assumption)`;
    ...
    // 验证
    const loginDiagnostics = vscode.languages.getDiagnostics(loginDoc.uri).filter(
      d => d.source === 'Semipilot'
    );
    const orderDiagnostics = vscode.languages.getDiagnostics(orderDoc.uri).filter(
      d => d.source === 'Semipilot'
    );
...
2. Email verification`;
    ...
    await editor.edit(editBuilder => {
      ...
      const range = new vscode.Range(
        ...
        new vscode.Position(lastLine, lineText.length)
      );
...
- 失败重试最多 3 次`;
    ...
    const codeActionProvider = new AssumptionCodeActionProvider();
    ...
    assert.ok(
      lines.some(line => line.includes('支付失败时系统发送重试通知') && !line.includes('(Assumption)')),
      'Should remove (Assumption) from accepted line'
    );
    assert.ok(
      lines.some(line => line.includes('支付超时时间为 30 秒') && line.includes('(Assumption)')),
      'Should keep other (Assumption) markers'
    );
 ...
 * 等待 Diagnostic 出现
 */
async function waitForDiagnostics(uri: vscode.Uri, timeout: number = 2000): Promise<void> {
