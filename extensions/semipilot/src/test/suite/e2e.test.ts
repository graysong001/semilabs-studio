...
`; */
      ...
      try {
        ...
        if (activeEditor) {
          assert.ok(
            activeEditor.document.uri.fsPath.includes('spec-task-test-001.md'),
            'Correct document should be opened'
          );
    ...
    test('Scenario: 智能排序验证', async () => {
      ...
      if (workspaceFolders && workspaceFolders.length > 0) {
        ...
        taskFiles.forEach(file => {
          assert.ok(
            file.fsPath.includes('spec-task-'),
            'Task file should follow naming convention'
          );
        });
      }
    });
