...
export interface TribunalCardProps {
  vetoReason: string;
  vetoRequirement?: string;
  fixSummary?: string;
  targetFile: string;
  workflowState: 'REJECTED' | 'FIXING';
  onViewDiff?: (targetFile: string) => void;
  onApproveFix: (targetFile: string) => void;
}

export const TribunalCard: React.FC<TribunalCardProps> = ({
  vetoReason,
  vetoRequirement,
  fixSummary,
  targetFile,
  ...
  return (
    <div style={{
      ...
      <div style={styles.banner}>
        ...
        <span style={styles.bannerText}>Architecture Conflict Detected</span>
        <span style={styles.bannerFile}>{getFileName(targetFile)}</span>
      </div>

      {/* 主体：Split View */}
      <div style={styles.splitView}>
        {/* Left: Archi's Challenge */}
        <div style={styles.leftPanel}>
          <div style={styles.panelHeader}>
            <span style={styles.panelIcon}>🛑</span>
            <span style={styles.panelTitle}>Archi's Challenge</span>
          </div>
          <div style={styles.panelContent}>
            <div style={styles.vetoReason}>
              <div style={styles.vetoLabel}>Reason:</div>
              <div style={styles.vetoText}>{vetoReason}</div>
            </div>
            {vetoRequirement && (
              <div style={styles.vetoRequirement}>
                <div style={styles.vetoLabel}>Requirement:</div>
                <div style={styles.vetoText}>{vetoRequirement}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={styles.rightPanel}>
          <div style={styles.panelHeader}>
            <span style={styles.panelIcon}>🔧</span>
            <span style={styles.panelTitle}>Poe's Fix</span>
          </div>
          <div style={styles.panelContent}>
            {workflowState === 'FIXING' && fixSummary ? (
              <div style={styles.fixSummary}>
                {fixSummary}
              </div>
            ) : (
              <div style={styles.fixPending}>
                <span style={styles.pendingIcon}>⏳</span>
                <span>Waiting for Poe to propose a fix...</span>
              </div>
            )}
          </div>
        </div>
