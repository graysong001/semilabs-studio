...
describe('Backend Integration Tests', () => {
    ...
    describe('Request Body Handling', () => {
        it('GET请求不应包含body', async function () {
            ...
            try {
                await messenger.request('chat/get-sessions', {
                    productId: 'test',
                });
                ...
                const isExpectedError = error.message.includes('Unknown message type') ||
                    error.message.includes('404') ||
                    error.message.includes('Not Found') ||
                    error.message.includes('500'); // Backend可能返回500
                assert.ok(isExpectedError, `应正确处理未实现的endpoint, got: ${error.message}`);
            }
        });
    ...
    describe('HTTP Headers', () => {
        ...
        it('应支持Authorization头（可选）', async function () {
            ...
            const authedMessenger = new SseMessenger_1.SseMessenger({
                baseUrl: backendUrl,
    ...
    describe('Error Response Handling', () => {
        it('应正确处理HTTP错误状态码', async function () {
            ...
            try {
                ...
                assert.ok(error.message.includes('HTTP') || error.message.includes('error'), `错误消息应包含HTTP信息: ${error.message}`);
            }
        });
        ...
        it('[错误提示] 网络连接失败应返回明确错误', async function () {
            ...
            try {
                ...
                assert.ok(error.message.includes('fetch') ||
                    ...
                    error.message.includes('network'), '错误信息应包含网络相关关键词');
            }
            finally {
                badMessenger.disconnect();
            }
...
//# sourceMappingURL=backend-integration.test.js.map